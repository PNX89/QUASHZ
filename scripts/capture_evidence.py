...
DEMO = ROOT / "examples" / "what_was_knowable.py"
...
def test_total() -> int:
    ...
    A total that counted only `tests` would understate the repository by every property test
    over the splitter and the controls, which is most of what defends the arithmetic.
    """
    total = 0
...
def main() -> int:
    ...
    slug = os.environ.get("GITHUB_REPOSITORY", "PNX89/QUASHZ")
    facts = {
        "tests": test_total(),
        "python": python_range(),
        "release": release(),
