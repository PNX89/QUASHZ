...
```bash
git clone https://github.com/PNX89/QUASHZ.git && cd QUASHZ
...
uv run python examples/what_was_knowable.py
uv run --group verdict python scripts/capture_evidence.py
```
...
- the splitter and the controls, against scikit-learn and DuckDB
- the verdict, issued again from the committed corpus
- the contract a vendor is handed, and what it does to a bad file
...
https://pnx89.github.io/QUASHZ, come from a real run whose output is committed under
